...
const SECTION_COLORS = ['#8c52ff','#5170ff','#a855f7','#6366f1','#7c3aed','#4f46e5','#9333ea','#3b82f6'];

export function getSectionColor(section: SectionNode | { id: string; color: string | null }): string {
...
export default function SectionListItem({ section, depth = 0 }: SectionListItemProps) {
  const { activeSectionId, setActiveSectionId, notebookId, refreshSections } = useNotebookWorkspace();
      ...
      >
        ...
        >
          {section.children.length > 0 || isCreatingChild ? (
            <ChevronRight size={13} style={{ transform: expanded ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.12s ease' }} />
          ) : (
