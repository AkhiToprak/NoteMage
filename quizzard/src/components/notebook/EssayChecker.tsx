...
import { X, Loader2, CheckCircle2, AlertTriangle, AlertCircle, FileText, SpellCheck } from 'lucide-react';
...
const TYPE_COLORS: Record<string, { bg: string; border: string; text: string; label: string }> = {
  spelling: { bg: 'rgba(239,68,68,0.06)', border: 'rgba(239,68,68,0.2)', text: '#fca5a5', label: 'Spelling' },
  grammar: { bg: 'rgba(251,191,36,0.06)', border: 'rgba(251,191,36,0.2)', text: '#fbbf24', label: 'Grammar' },
  clarity: { bg: 'rgba(96,165,250,0.06)', border: 'rgba(96,165,250,0.2)', text: '#60a5fa', label: 'Clarity' },
  structure: { bg: 'rgba(167,139,250,0.06)', border: 'rgba(167,139,250,0.2)', text: '#a78bfa', label: 'Structure' },
};

export default function EssayChecker({ notebookId, initialText, onClose, onApplyFix }: EssayCheckerProps) {
  ...
  const scoreColor = result
    ? result.overallScore >= 80 ? '#4ade80' : result.overallScore >= 60 ? '#fbbf24' : '#fca5a5'
    : '#c4a9ff';
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
            ...
            <>
              <textarea
                ...
                style={{
                  ...
                  fontSize: '14px', color: '#ede9ff', lineHeight: 1.7,
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '12px', justifyContent: 'flex-end' }}>
                <button
                  ...
                  style={{
                    ...
                    color: '#c4a9ff', fontSize: '13px', fontWeight: 600,
                    cursor: loading || !text.trim() ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit', opacity: loading || !text.trim() ? 0.5 : 1,
                    display: 'flex', alignItems: 'center', gap: '6px',
                  }}
                >
                  {loading ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <SpellCheck size={14} />}
                  Check Grammar
                </button>
                <button
          ...
          {loading && (
            <div style={{
              ...
              <Loader2 size={28} style={{ color: '#c4a9ff', animation: 'spin 1s linear infinite' }} />
              <span style={{ fontSize: '13px', color: 'rgba(237,233,255,0.4)' }}>Analyzing your text...</span>
            </div>
          )}
          ...
          {result && !loading && (
            <>
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#ede9ff', marginBottom: '4px' }}>
                    {result.overallScore >= 80 ? 'Great writing!' : result.overallScore >= 60 ? 'Good, with room for improvement' : 'Needs work'}
                  </div>
                  <div style={{ fontSize: '13px', color: 'rgba(237,233,255,0.5)', lineHeight: 1.5 }}>
                    {result.summary}
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', gap: '4px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      ...
                      border: !filterType ? '1px solid rgba(140,82,255,0.4)' : '1px solid rgba(255,255,255,0.06)',
              ...
              {result.issues.length === 0 ? (
                <div style={{
                  ...
                  <span style={{ fontSize: '14px', color: '#4ade80', fontWeight: 600 }}>No issues found!</span>
                </div>
              ) : (
                  ...
                  {filteredIssues.map((issue, i) => {
                    ...
                    return (
                      <div key={i} style={{
                          ...
                          {onApplyFix && issue.original && issue.suggestion && (
                            <button
                              ...
                              style={{
                                ...
                                border: `1px solid ${colors.border}`,
                                background: 'transparent',
                                color: colors.text, fontSize: '10px', fontWeight: 600,
                        ...
                        {issue.original && (
                          <div style={{
                            fontSize: '13px', color: 'rgba(237,233,255,0.5)',
                        ...
                        {issue.suggestion && (
                          <div style={{
                            fontSize: '13px', color: '#ede9ff', fontWeight: 500,
                            marginBottom: '4px',
                          }}>
                            → {issue.suggestion}
                          </div>
                        ...
                        <div style={{
                          fontSize: '12px', color: 'rgba(237,233,255,0.4)', lineHeight: 1.5,
                        }}>
                          {issue.explanation}
                        </div>
                      </div>
