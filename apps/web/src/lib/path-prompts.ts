...
export function buildPathStructurePrompt(ctx: PathStructureContext): string {
  const lines: string[] = [
    ...
    '',
    'JSON output shape (the keys MUST match this exactly — `phases`, not `sections`):',
    ...
    'The UI renders each phase as a "Section" — but the JSON key stays `phases`.',
    '',
    'Rules:',
    '- Output 3–6 sections.',
...
export function buildTheoryPrompt(ctx: SlotContentContext): string {
  const lines: string[] = [
    ...
    '',
    'JSON output shape (keys MUST match EXACTLY — camelCase, no snake_case):',
    '{ "title": string, "introduction": string, "keyPoints": string[], "examples": [ { "label": string, "explanation": string } ], "summary": string? }',
    '`keyPoints` must be a real JSON array of plain strings (never an object keyed by index, never a stringified blob). `examples` must be a real array of `{label, explanation}` objects.',
    '',
...
export function buildFlashcardsPrompt(ctx: SlotContentContext): string {
  const lines: string[] = [
    ...
    '`flashcards` MUST be a non-empty JSON array of `{question, answer}` objects (8–12 cards). Never stringified, never keyed by index.',
    '',
...
export function buildQuizPrompt(ctx: SlotContentContext): string {
  ...
  const lines: string[] = [
    ...
    '',
    'JSON output shape (top-level keys MUST match EXACTLY — camelCase, no snake_case):',
    '{ "title": string, "questions": [ { "kind": <one of the 11 enum values below>, "prompt": string, "hint": string?, "correctExplanation": string?, "wrongExplanation": string?, "payload": <kind-specific object, shapes specified below> } ] }',
    'Use `correctExplanation` / `wrongExplanation` — NEVER `correct_explanation` / `wrong_explanation`. Use `correctIndex` — NEVER `correct_index`. Use `acceptableAnswers` — NEVER `acceptable_answers`. ALL keys are camelCase.',
    '',
