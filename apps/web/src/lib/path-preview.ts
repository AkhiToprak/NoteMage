...
/** First try + one repair (D6). On a second miss the orchestrator throws and the
 *  caller falls back to the sample flow. */
 ...
 * (on failure) re-call once with the failure summary folded back in as a
 * corrective notice. Generic so any artifact (theory, questions, …) reuses it,
 * and so the full-path generators can adopt the same loop later. Returns
 * `data: null` once `attempts` are spent — the caller decides the fallback.
 */
export async function generateWithRepair<T>(opts: {
  ...
  const max = Math.max(1, opts.attempts ?? PREVIEW_ATTEMPTS);
