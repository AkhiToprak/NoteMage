 ...
 *  `[x, y, w, h]`-style fractional bbox reinterpreted as corners, + caption.
 *  Values above 1 (percent / 0–1000-grid / pixel drift — possibly PER AXIS)
 *  pass through untouched: only the crop stage knows the page dimensions, so
 *  `cropFigure` owns all scale rescue. */
function normalizeImageBlock(block: Record<string, unknown>): void {
  if (Array.isArray(block.bbox)) {
    ...
    if (bbox.length === 4 && bbox.every((v) => Number.isFinite(v) && v <= 1.2)) {
      const [x0, y0] = bbox;
      let [, , x1, y1] = bbox;
