...
/* Dashboard (Web). The Figma redesign layout, wired to the learner's cached
   path cockpit via /api/dashboard. The active path drives the hero, the study
   plan, the review queue and the next-checkpoint rail. */
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/dashboard')
      ...
      .then((body) => {
        if (cancelled) return;
        setState({ kind: 'ready', data: body.data as DashboardData });
      })
      .catch(() => {
    ...
    return (
      <AppShell>
        <div className={ui.card} style={{ marginTop: 30, padding: '52px 32px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: 'var(--body)' }}>Loading your dashboard…</div>
        </div>
      </AppShell>
    );
  }
  ...
  if (state.kind === 'error' || !state.data?.hasUsablePath || !state.data.active) {
  ...
  const sessionsDone = state.data.studiedToday ? 1 : 0;
