...
type DashState =
  | { kind: 'loading' }
  | { kind: 'error' }
  | { kind: 'ready'; paths: SerializedPath[] };
...
export default function PathsPage() {
  ...
  const refresh = () => {
    fetch('/api/learn/paths')
      ...
      .then((body) => {
        const paths: SerializedPath[] = Array.isArray(body?.data) ? body.data : [];
        setState({ kind: 'ready', paths });
      })
    ...
    return (
      <AppShell>
        <div className={ui.card} style={{ marginTop: 30, padding: '52px 32px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: 'var(--body)' }}>Loading your paths…</div>
        </div>
      </AppShell>
    );
  }
      ...
      {deleteTarget && (
        <DeletePathDialog
          ...
          onDeleted={() => {
            ...
            setState((prev) =>
              prev.kind === 'ready'
                ? { kind: 'ready', paths: prev.paths.filter((p) => p.id !== deleteTarget.id) }
                : prev,
            );
          }}
        />
