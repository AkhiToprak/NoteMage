...
/* WEX01 — Exams (Web). Prepare with paths, weak-point practice and readiness
   tracking. Wired to the learner's real exams via /api/user/exams/overview:
   upcoming exams (enriched with the same readiness rollup the exam overview
   page uses) drive the Active cards; past exams fill Archived. "Create exam"
   points at the standalone /exams/new flow (Exam Mode Phase 0). */
...
export default function ExamsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/user/exams/overview')
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error(String(res.status)))))
      .then((body) => {
        if (!cancelled) setState({ kind: 'ready', data: body.data as Overview });
      })
      .catch(() => {
  ...
  return (
    <AppShell>
      ...
      {state.kind === 'loading' && <StatusNote icon="hourglass_empty" text="Loading your exams…" />}
      {state.kind === 'error' && <StatusNote icon="error" text="Couldn't load your exams. Please try again." />}
      {state.kind === 'ready' && <Content data={state.data} />}
    </AppShell>
  );
}
