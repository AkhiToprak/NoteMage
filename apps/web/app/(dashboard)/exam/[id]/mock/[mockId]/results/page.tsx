...
interface MockDetail {
  mock: {
    id: string;
    status: string;
    config: { type: MockType; questionCount: number; durationSec: number };
  ...
  result: {
    ...
    breakdown: {
      byType: BreakdownGroup[];
      byTopic: BreakdownGroup[];
      strongest: BreakdownGroup | null;
      weakest: BreakdownGroup | null;
    };
  } | null;
}
...
export default function MockResultsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        // Not taken yet → send the learner into the run.
        if (d.mock.status !== 'completed' || !d.result) {
          router.replace(`/exam/${examId}/mock/${mockId}`);
          return;
        }
        setDetail(d);
        setStatus('ready');
  ...
  if (status !== 'ready' || !detail || !detail.result) {
    return (
      <AppShell>
        <div className={s.center}>
          <span className="material-symbols-outlined" aria-hidden style={{ fontSize: 38, color: 'var(--muted)' }}>
            {status === 'notfound' ? 'event_busy' : 'hourglass_empty'}
          </span>
          <p style={{ margin: 0, fontSize: 15, color: 'var(--body)' }}>
            {status === 'notfound' ? 'Mock exam not found.' : status === 'error' ? 'Couldn’t load these results.' : 'Loading results…'}
          </p>
          {status !== 'loading' ? (
  ...
  const preset = MOCK_TYPE_PRESETS[mock.config.type] ?? MOCK_TYPE_PRESETS.quick;
