...
/* Exam Mode Phase 5 — Post-Exam Learning Report (client view).
 * Receives pre-resolved ExamReportView from the server page — no fetch, no
 * loading spinner. useRegisterMageContext stays here because it is a useEffect
 * that writes into the Mage context provider (client-only). */
...
export interface ExamReportViewProps {
  examId: string;
  data: ExamReportView;
}

export default function ExamReportViewPage({ examId, data }: ExamReportViewProps) {
  useRegisterMageContext({ type: 'exam', ids: { examId }, title: data.exam.title });
