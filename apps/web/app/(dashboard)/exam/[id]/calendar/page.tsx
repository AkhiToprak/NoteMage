...
export default function StudyCalendarPage() {
  ...
  if (status === 'loading') {
    return (
      <AppShell><CenteredMessage icon="hourglass_empty" title="Loading your calendar…" /></AppShell>
    );
  }
  if (status === 'notfound') {
    return (
    ...
    return (
      <AppShell>
        <CenteredMessage
          ...
          action={<button type="button" className={`${ui.btn} ${ui.secondary}`} onClick={() => { setStatus('loading'); void load(); }}>Retry</button>}
        />
      </AppShell>
    );
  }
