...
interface TodaysPlanViewProps {
  examId: string;
  /** Plan resolved on the server (same shape the /plan GET returned). Null only
   *  when the plan load failed (errored); a genuine not-found is a 404 upstream. */
  initialData: PlanResponse | null;
  errored: boolean;
}

export default function TodaysPlanView({ examId, initialData, errored }: TodaysPlanViewProps) {
    ...
    return (
      <AppShell>
        <CenteredMessage
          ...
          action={<Link href={`/exam/${examId}/plan`} className={`${ui.btn} ${ui.secondary}`}>Retry</Link>}
        />
      </AppShell>
    );
  }
