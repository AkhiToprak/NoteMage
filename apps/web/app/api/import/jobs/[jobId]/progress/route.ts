...
// Phase 3 of the OneNote-import plan — a format-neutral SSE progress stream.
//
// A structural copy of the PDF route
// (`app/api/notebooks/[id]/pdf-import/[jobId]/progress/route.ts`), but keyed by
// `jobId` + `userId` alone — no notebook in the path — so it serves any
// `ImportJob` the user owns regardless of `sourceFormat`. The PDF route is left
// untouched; this one exists so the OneNote modal can stream without a notebook
// segment and so the `done` payload can carry the OneNote `summary`.
//
...
//       — PDF jobs populate `pageId`; OneNote jobs populate `summary`.
//
//   event: error
//   data: { message }
//
// Both workers write `ImportJob.progress` after each page. This handler polls
// the row every 500ms, diffs against the last snapshot, and emits a `progress`
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        console.error('[import-job progress SSE]', error);
        try {
